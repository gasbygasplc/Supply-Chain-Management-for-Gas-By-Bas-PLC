...
useEffect(() => {
    ...
    const fetchConsumers = async () => {
        try {
            const response = await axios.get(`http://localhost:4000/api/auth/consumers?outletId=${outletId}`);
    ...
    const handleSubmitGasRequest = async (e) => {
        ...
        try {
            const response = await axios.post("http://localhost:4000/api/gas/request", requestPayload);
    
            if (response.data.success) {
                toast.success("Gas request placed successfully!");
                setGasRequest({ userId: "", gasType: "", quantity: 1, priorityLevel: "Standard" });
            ...
            console.error("Error placing gas request:", error.response?.data || error.message);
    ...
    {filteredConsumers.length > 0 ? (
        filteredConsumers.map((consumer) => (
            <tr key={consumer._id}>
                <td className="border border-gray-300 px-4 py-2">{consumer.name}</td>
                <td className="border border-gray-300 px-4 py-2">{consumer.nic}</td>
                <td className="border border-gray-300 px-4 py-2">{consumer.phone}</td>
                <td className="border border-gray-300 px-4 py-2">{consumer.email}</td>
            </tr>
        ))
    ) : (
        <tr>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
        <div className="bg-white p-6 rounded shadow-lg w-[90%] sm:w-[400px]">
            ...
            <form onSubmit={handleSubmitGasRequest} className="flex flex-col gap-4">
                <select
                    name="gasType"
                    value={gasRequest.gasType}
                    onChange={(e) => setGasRequest({ ...gasRequest, [e.target.name]: e.target.value })}
                    required
                    className="w-full p-2 border rounded"
                >
