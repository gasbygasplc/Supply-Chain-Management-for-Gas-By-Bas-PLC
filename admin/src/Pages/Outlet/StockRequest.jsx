  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(
        "https://mw.gasbygas.store/api/outlet/request-delivery",
        payload,
        { headers: { Otoken } }
      );
  ...
  return (
    <div className="w-full flex flex-col min-h-full">
      ...
      <div className=" text-gray-700 text-base mt-6 rounded-md py-6 px-4  md:px-6 mx-4 md:mx-[2%] bg-white border">
        <h1 className="font-semibold text-2xl md:text-2xl mb-4">
          Your Delivery Shedule Request
        </h1>
            ...
            <tbody>
              {gasSheduleReq.map((req, index) => (
                <tr key={index} className="border-t">
                  ...
                  <td className="px-4 py-2">
                    {new Date(req.expectedDeliveryDate).toLocaleDateString()}
                  </td>
                  <td className="px-4 py-2">
                    <span
                      className={`text-xs font-medium me-2 px-[13px] py-[5px] rounded-full ${
                        req.status === "Pending"
