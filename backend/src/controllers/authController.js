...
export const registerUser = async (req, res) => { //register user function
    ...
    try {
        ...
        if (!/^\d{9}$/.test(normalizedPhone)) {
            return res.status(400).json({ success: false, message: "Invalid phone number format." }); // return an error if the phone number is invalid
        }

        const existingUser = await User.findOne({ $or: [{ nic }, { phone: normalizedPhone }, { email }] }); //check if the user already exists nic phone and email
        if (existingUser) {
            return res.status(400).json({ success: false, message: "User already exists." }); // return an error if the user already exists
        }
...
export const loginUser = async (req, res) => {
    ...
    try {
        ...
        if (!user) return res.status(404).json({ message: "User not found." }); // check the  user if is not found return an error

        const isPasswordValid = await bcrypt.compare(password, user.password); // compare the password with the stored hashed password

        if (!isPasswordValid) return res.status(401).json({ message: "Invalid credentials." }); // check the password if is not valid return an error

        const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: '1h' }); // create a token using with user id and secret key,  token is set to expire in one hour 
        res.status(200).json({ message: "Login successful.", token, user }); // send the token and user details as response
    } catch (error) {
        console.error("Error during login:", error);
        res.status(500).json({ message: "Error logging in.", error }); // return an error if there is an error during the login
    }
};
//=========================================================================================
