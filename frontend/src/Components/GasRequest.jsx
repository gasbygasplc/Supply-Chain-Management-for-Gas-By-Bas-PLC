...
const GasRequest = () => {
  ...
  useEffect(() => {
    if (userData.role === "Organization" && organizationGasDetails.length > 0) {
      setSelectedOrgGas(organizationGasDetails[0]);
      console.log(selectedOrgGas); //set the first gas type as the selected gas type
    } else {
      handleGasSelection("Small");
    }
  ...
  const handleSaveOrder = async () => {
    ...
    const totalGasOrders = gasOrder.reduce(
    ...
    const selectedOutletData = filteredOutlets.find(
      (outlet) => outlet._id === selectedOutlet
    );
    const selectedOutletName = selectedOutletData
      ? selectedOutletData.outletName
      : "Unknown Outlet";
  ...
  useEffect(() => {
    const districts = [
      ...new Set(outletlocation.map((location) => location.district)),
    ];
  ...
  const handleCitySelection = async (e) => {
    ...
    try {
      const response = await axios.get(
        `https://mw.gasbygas.store/api/outlet/api/outlets/${city}`
      );
      ...
      <div className="flex flex-col w-full md:flex-row justify-between items-center sm:items-start gap-4 px-4 py-4 bg-white rounded-lg md:px-8 md:py-8 md:gap-8">
        ...
        <div className="w-full">
            ...
            <img
              ...
              src={
                userData.role === "Organization"
                  ? selectedOrgGas?.image || asstets.small_gas
                  : gasDetails?.image || asstets.small_gas
              }
            />
          </div>
          ...
          {userData.role === "Organization" ? (
            <div className="flex w-full justify-between items-center gap-4 mt-6">
              {organizationGasDetails.map((gas) => (
                <div
                  key={gas.type}
                  onClick={() => setSelectedOrgGas(gas)}
                  className={`w-full py-4 bg-white border 
                                            ${
                                              selectedOrgGas?.type === gas.type
                                                ? "border-primary"
                                                : "border-gray-300"
                                            }
                                            rounded-lg flex justify-center items-center cursor-pointer`}
                >
                  <img className="w-full" src={gas.image} alt={gas.type} />
                </div>
              ))}
            </div>
          ) : (
        ...
        <div className="w-full flex flex-col gap-5">
          <div className="flex justify-between items-center">
            <p>Weight</p>
            <p>{userData.role === "Organization"? selectedOrgGas?.weightKG + "Kg": gasDetails?.weightKG + "Kg"}</p>
          </div>
              ...
              <img
                ...
                onClick={() => {
                  const maximumQuantity =
                    userData.role === "Organization" ? 10 : 2;
                  if (gasQuantity < maximumQuantity) {
                    updateGasQuantity("+");
                  } else {
                    toast.error(
                      `You cannot add more than ${maximumQuantity} gases.`
                    );
                  }
                }}
                src={asstets.add}
          ...
          <div className="flex justify-between items-center">
            <p>
              LKR{" "}
              {userData.role === "Organization" ? (selectedOrgGas?.price * gasQuantity).toFixed(2) :  (gasDetails?.price * gasQuantity).toFixed(2)}
            </p>
            <p>
              LKR{" "}
              {userData.role === "Organization" ? (selectedOrgGas?.price * gasQuantity).toFixed(2) : (gasDetails?.price * gasQuantity).toFixed(2)}
        ...
        <div className="w-full flex flex-col gap-8">
          <div className="flex flex-col justify-center gap-4">
            ...
            <p className="text-sm leading-6">
              Gas By Gas is a brand who provides LPG gas cylinders for homes and
              organizations. Their quality is much better than others.
            </p>
          </div>
